...
/* $(document).on('click', '#editForm<?php echo $var; ?>', function(){
	$('#form<?php echo $var; ?>').slideUp();
	reset();
	document.getElementsByTagName("form")[0].setAttribute("id", "formInput<?php echo $var; ?>");
	$('#form<?php echo $var; ?>').fadeIn(1000);
...
/* $(document).on('submit', '#editForm<?php echo $var; ?>', function() {
	$('#idImgLoader').show();
    $.post("<?php echo $uri; ?>act.php?act=edit", $(this).serialize())
        .done(function(data) {
			reset();
            $('#form<?php echo $var; ?>').slideUp(500, 'swing');
			$('#idAlert<?php echo $var; ?>').html(data);
...
function editForm(id){
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            $('#idKd<?php echo $var; ?>').val(data.kd_karyawan);
            $('#idNama<?php echo $var; ?>').val(data.nm_karyawan);
...
function hapusData(act,id) {
 ...
 function toggle() {
 if( document.getElementById("hidethis<?php echo $var; ?>").style.display=='none' ){
   document.getElementById("hidethis<?php echo $var; ?>").style.display = '';
 }else{
   document.getElementById("hidethis<?php echo $var; ?>").style.display = 'none';
 }
} 
...
/* function toggleTable()
    ...
    var status = document.getElementById("loginTable<?php echo $var; ?>").style.display;

    if (status == 'block') {
      document.getElementById("loginTable<?php echo $var; ?>").style.display="none";
    } else {
      document.getElementById("loginTable<?php echo $var; ?>").style.display="block";
    }
  } */
